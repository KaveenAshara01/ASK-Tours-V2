        ...
        return (
            <div className="bg-white rounded-xl shadow p-6">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={() => { setEditingEvent(null); setView('form'); }} className="bg-primary-600 text-white px-4 py-2 rounded-lg hover:bg-primary-700">+ Add Event</button>
                </div>
                {loading ? <p>Loading...</p> : (
                        ...
                        <div className="flex flex-wrap gap-2">
                            {previews.images.map((p, i) => (
                                <div key={i} className="relative w-24 h-24">
                                    ...
                                    <button type="button" onClick={() => removePreview('images', i)} className="absolute top-0 right-0 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs">x</button>
                                </div>
                            ))}
                        </div>
                    </div>
